...
ALTER TABLE LOS_HEREDEROS_DE_MONTIEL_Y_EL_DATO_PERSISTIDO.PROVINCIA_X_LOCALIDAD
    ADD CONSTRAINT UQ_PROVINCIA_X_LOCALIDAD UNIQUE (provincia, localidad)
GO

ALTER TABLE LOS_HEREDEROS_DE_MONTIEL_Y_EL_DATO_PERSISTIDO.BARRIO_X_LOCALIDAD
    ADD CONSTRAINT UQ_BARRIO_X_LOCALIDAD UNIQUE (barrio, localidad)
GO
...
BEGIN
    ...
    SELECT DISTINCT Persona.id, m.VENTA_CODIGO
    FROM gd_esquema.Maestra m
...
BEGIN
    ...
    SELECT DISTINCT Persona.id, m.ALQUILER_CODIGO
    FROM gd_esquema.Maestra m
             JOIN LOS_HEREDEROS_DE_MONTIEL_Y_EL_DATO_PERSISTIDO.PERSONA Persona
...
BEGIN
    ...
    SELECT DISTINCT Persona.id, m.INMUEBLE_CODIGO
    FROM gd_esquema.Maestra m
             JOIN LOS_HEREDEROS_DE_MONTIEL_Y_EL_DATO_PERSISTIDO.PERSONA Persona
...
BEGIN
    ...
    SELECT DISTINCT m.VENTA_CODIGO,
                    (SELECT persona_id
                     FROM LOS_HEREDEROS_DE_MONTIEL_Y_EL_DATO_PERSISTIDO.COMPRADOR Comprador
                     WHERE Comprador.codigo_venta = m.VENTA_CODIGO) AS comprador_id,
                    m.VENTA_FECHA,
